...
def print_board(grid):
	...
	for big_i in range(4):
		for big_j in range (4):
			base = square(big_i * (size//4) + (size//8), big_j * (size//4), (size//8), black, base)
			base = square(big_i * (size//4), big_j * (size//4) + (size//8), (size//8), black, base)
...
# take a checkers game and make a legal move
def make_move(grid, old_i, old_j, new_i, new_j):
	# i is vertical positions
	# j is horizontal positions
	# grid is 8x8 so values range from 0 to 7
	for val in [old_i, old_j, new_i, new_j]:
		if val < 0 or val > 7:
			print("INVALID MOVE:  Indices outside of 8x8 gameboard\n")
			return grid
	if (grid[old_i][old_j] == 0):
		print("INVALID MOVE:  No piece at location (" + str(old_i) + "," + str(old_j) + ")\n")
		return grid
	if abs(old_i - new_i) == 1 or abs (old_j - new_j) == 1:
		if (grid[new_i][new_j] != 0):
			print("INVALID MOVE:  Already a  piece at location (" + str(new_i) + "," + str(new_j) + ")\n")
			return grid
		grid[new_i][new_j] = grid[old_i][old_j] 
		grid[old_i][old_j] = 0
		return grid
	if abs(old_i - new_i) == 2 or abs (old_j - new_j) == 2:
		to_jump = grid[(old_i + new_i)//2][(old_j + new_j)//2]
		if to_jump == 0:
			print("INVALID MOVE:  No piece to jump\n")
			return grid
		if (grid[new_i][new_j] != 0):
			print("INVALID MOVE:  Already a  piece at location (" + str(new_i) + "," + str(new_j) + ")\n")
			return grid
		if grid[old_i][old_j] == to_jump:
			print("INVALID MOVE:  Can't jump your own pieces\n")
			return grid
		grid[new_i][new_j] = grid[old_i][old_j]
		grid[old_i][old_j] = 0
		grid[(old_i + new_i)//2][(old_j + new_j)//2] = 0
	return grid
